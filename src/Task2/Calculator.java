...
/**
 * Created by Владислав on 08.02.2018.
 */
public class Calculator {
    ...
    public static int getPrioritet(char s){
        int preoritet = 0;
        switch (s){
            case '(': preoritet = 1;break;
            case '+': preoritet = 2;break;
            case '-': preoritet = 2;break;
            case '*': preoritet = 3;break;
            case '/': preoritet = 3;break;
            case '^': preoritet = 4;break;
        }
        return preoritet;
    }
    ...
    public static float calculate(StringBuilder stringBuilder){
        ...
        for(int i=0;i<mas.length;i++){
            /*if(mas[i].equals("/") & Float.parseFloat(mas[i+1])==0.0f){
                System.out.println("Невалидная запись, деление на 0");
                System.exit(0);
            }*/
            Matcher matcher1 = pattern1.matcher(mas[i]);
            if(matcher1.matches()){
